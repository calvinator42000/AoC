...
def main(data):
    face_list = ('J','2','3','4','5','6','7','8','9','T','Q','K','A')
...
def getScore(hand, face_list):
    replaced_hand = hand.replace('J',getFrequentChar(hand))
    sorted_replaced_hand = "".join(sorted(list(replaced_hand), key=lambda x: face_list.index(x)))
    regex_list = [r'(.)\1{4}', r'(.)\1{3}', r'(.)\1{1}(.)\2{2}|(.)\3{2}(.)\4{1}', r'(.)\1{2}', r'.?(.)\1.?(.)\2.?', r'(.)\1', r'.*']
    return 7 - (list(map(lambda x: re.search(x, sorted_replaced_hand) != None, regex_list)).index(True))

def getFrequentChar(string):
    filtered_string = list(string.replace('J',''))
    if len(filtered_string) == 0:
        return 'A'
    return max(filtered_string, key = filtered_string.count)
